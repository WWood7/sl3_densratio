...
in tackling covariates shifting. `sl3` is equipped with learners that can tackle density ratios 
written in the form $\frac{f(X)}{g(X)}$ or $\frac{f(X\mid Y)}{g(X\mid Y)}$.
...
$S$ to denote `smoked`. The density ratios to be estimated here are $\frac{p(W,L\mid S=1)}{p(W,L\mid S=0)}$ 
and $\frac{p(W\mid L, S=1)}{p(W\mid L, S=0)}$, suppose $p$ is the true PDF. The goal is, 

Here we see $S$ as an indicator of marginal distributions, i.e. $p(W,L\mid S=s)$ with different `s` 
...
## Estimation of Marginal Density Distributions

We now start our estimation process. To implement any learner in `sl3`, covariates, an outcome and an 
`sl3_Task` object are needed. In our case, the covariates are `birthwt` and `birthlen`, the tricky part 
is that we set `smoked` as our outcome.
